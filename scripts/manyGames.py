...
def main(username="oneisone"):
	games = []
	totalaverage = 0
	c = 0
	userGames = list(lichess.api.user_games(str(username), max=50, perfType="rapid", format=PYCHESS, auth=str(os.getenv("LICHESSAUTH"))))
	for game in userGames:
		ga = GameAnalyzer(game=game)
